...
class IndexController extends Controller {
	...
	*/
	...
	public function index()
	{
		...
		$tintuc_moinhat = DB::table('news')->where('com', 'tin-tuc')->where('status',1)->orderBy('created_at','desc')->first();
		$com='index';
	...
	public function getProductDetail($id)
	{
		...
		if(!empty($product_detail)){
			$banner_danhmuc = DB::table('lienket')->select()->where('status',1)->where('com','chuyen-muc')->where('link','san-pham')->get()->first();
			$albums_project = DB::table('album_project')->where('project_id', $product_detail->id)->orderBy('stt','asc')->get();
			...
			$com = 'detail';
			// End cấu hình SEO
			return view('templates.product_detail_tpl', compact('product_detail','banner_danhmuc','keyword','description','title','img_share','albums_project','album_hinh','cateProduct','productSameCate','tintucs','cate_pro', 'com'));
		}else{
			return redirect()->route('getErrorNotFount');
		}
	}
    ...
    public function getDetailService($alias)
    {
    	$service = DB::table('news_categories')->where('alias', $alias)->first();
    	$com = 'service';
    	return view('templates.detailService', compact('service', 'com'));
    }
}
